...
app.get('/', (request, response) => {
	response.sendFile(path.resolve(__dirname, 'menu.html'))
})
app.get('/jugar', (request, response) => {
	//response.sendFile(path.resolve(__dirname, 'juego.html'))
	const titulo = fs.readFileSync(path.resolve(__dirname, 'preguntas', 'programacion.txt'), 'utf8').split('\n')[0];
...
app.get('/pedir', (request, response) => {
	const preguntas = [];
